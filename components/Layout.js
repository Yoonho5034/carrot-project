...
const Layout = ({ title, canGoBack, hasTabBar = true, children }) => {
  ...
  return (
    <div>
      <div
        className={`bg-white flex items-center  py-2 fixed w-full max-w-lg border-b-2 font-semibold text-gray-600  ${
          canGoBack ? "justify-start" : "justify-center"
        }`}
      >
        {canGoBack && (
          <button className="ml-4" onClick={goToBack}>
            &larr;
          </button>
        )}
        {title && <span>{title}</span>}
      </div>
      <div className={hasTabBar ? "pt-16 pb-24" : "py-16"}>{children}</div>
      {hasTabBar && (
